...
public class TagReader {
    ...
    public static Tag read(String fileName) {
        ...
        String cmd = Settings.getAtomicParsley() + " \"" + currentTag.getFilename() + "\" -t +";

        garethflowers.retagit.process.Executer c = new garethflowers.retagit.process.Executer();
        c.ExecCommand(cmd);
        ...
        for (int i = 0; i < result.length; i++) {
            ...
            if (value.startsWith("Atom \"")) {
                if (value.substring(10, 22).equals("\" contains: ") || value.substring(11, 23).equals("\" contains: ")) {
                    String tag[] = value.split("\"");
                    String contents = value.substring(22, value.length()).trim();
                    ...
                    if (tag[1].endsWith(COPYRIGHT + "alb")) {
                        ...
                        if (tracks.length > 0) {
                            try {
                                currentTag.setTrack(Integer.parseInt(tracks[0]));
                            } catch (NumberFormatException ex) {
                                System.out.println(ex.getStackTrace());
                            }
                        }
                        if (tracks.length > 1) {
                            try {
                                currentTag.setTrackTotal(Integer.parseInt(tracks[1]));
                            } catch (NumberFormatException ex) {
                                System.out.println(ex.getStackTrace());
                            }
                        }
                        ...
                        if (contents.equals("Movie") || contents.equals("Short Film")) {
                            currentTag.setMediaType(1);
                        } else if (contents.equals("Music Video")) {
                            currentTag.setMediaType(2);
                        } else if (contents.equals("TV Shows") || contents.equals("TV Show")) {
                            currentTag.setMediaType(3);
                        }
                    } else if (tag[1].equals("rtng")) {
                        if (contents.equals("clean") || contents.equals("Clean Content")) {
                            currentTag.setAdvisory(1);
                        } else if (contents.equals("explicit") || contents.equals("Explicit Content")) {
                        ...
                        try {
                            currentTag.setTvSeasonNum(Integer.parseInt(contents));
                        } catch (NumberFormatException ex) {
                            System.out.println(ex.getStackTrace());
                        }
                    } else if (tag[1].equals("tves")) {
                        try {
                            currentTag.setTvEpisodeNum(Integer.parseInt(contents));
                        } catch (NumberFormatException ex) {
                            System.out.println(ex.getStackTrace());
                        }
                        ...
                        try {
                            currentTag.setReleaseDate(Integer.parseInt(contents.substring(0, 4)));
                        } catch (NumberFormatException ex) {
                            System.out.println(ex.getStackTrace());
                        }
                    } else if (tag[1].endsWith(COPYRIGHT + "gen") || tag[1].equals("gnre")) {
                        currentTag.setGenre(contents);
                    } else if (tag[1].equals("disk")) {
                        String discs[] = contents.split(" of ");
                        if (discs.length > 1) {
                            try {
                                currentTag.setDiscTotal(Integer.parseInt(discs[1]));
                            } catch (NumberFormatException ex) {
                                System.out.println(ex.getStackTrace());
                            }
                ...
                } else if (value.startsWith("Atom \"----\" [com.apple.iTunes;iTunEXTC] contains: ")) {
                    String[] contents = value.replace("Atom \"----\" [com.apple.iTunes;iTunEXTC] contains: ", "").split("\\|");
